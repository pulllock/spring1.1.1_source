 ...
 * Bean后处理器，定制bean
 * BeanFactory在创建每一个bean实例时，在任何初始化方法（afterPropertiesSet和init-method属性）被调用之前和
 * 之后，post-processor将会从BeanFactory分别得到一个回调
 *
 * 可以对bean做任何操作
 *
 * BeanFactory和ApplicationContext对待post-processor有些不同，ApplicationContext会自动监测并注册
 * BeanFactory得使用代码显式注册
 */
